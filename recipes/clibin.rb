...
[node[:oracle_db][:ora_base], node[:oracle_db][:client][:install_dir]].each do |dir|
  directory dir do
    owner 'oracli'
    group 'oracli'
    mode '0755'
...
node[:oracle_db][:client][:install_files].each do |zip_file|
...
# Fixed a compile error while installing the client
  execute "sed_client_cvu_config" do
    ...
    user "oracli"
    group 'oracli'
    cwd "#{node[:oracle_db][:client][:install_dir]}/client/stage/cvu/cv/admin"
  end
end
...
# Filesystem template.
template "#{node[:oracle_db][:client][:install_dir]}/cli11R23.rsp" do
  owner 'oracli'
  group 'oracli'
  mode '0644'
end
...
bash 'run_client_installer' do
  cwd "#{node[:oracle_db][:client][:install_dir]}/client"
  environment (node[:oracle_db][:client][:env])
  code "sudo -Eu oracli ./runInstaller -showProgress -silent -waitforcompletion -ignoreSysPrereqs -responseFile #{node[:oracle_db][:client][:install_dir]}/cli11R23.rsp -invPtrLoc #{node[:oracle_db][:ora_base]}/oraInst.loc"
  returns [0, 6]
end

execute 'root.sh_client' do
  command "#{node[:oracle_db][:client][:ora_home]}/root.sh"
end
...
ruby_block 'set_client_install_flag' do
  block do
    node.set[:oracle_db][:client][:is_installed] = true
  end
  action :create
end
...
# target database.
cookbook_file "#{node[:oracle_db][:client][:ora_home]}/network/admin/tnsnames.ora" do
  owner 'oracli'
  group 'oracli'
  mode '0644'
end
...
cookbook_file "#{node[:oracle_db][:client][:ora_home]}/sqlplus/admin/glogin.sql" do
  owner 'oracli'
  group 'oracli'
  mode '0644'
end
